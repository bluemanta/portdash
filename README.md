...
- **Restarts a launchd job properly**, which is usually what you wanted from one anyway: a service kept alive by a LaunchAgent needs restarting to pick up a code change, and PortDash asks launchd to do it (`launchctl kickstart -k`) rather than stopping it and starting a copy of its own. launchd stops its instance and starts the replacement as one operation, so there's no moment for two of them to be running. Apple's own agents are left out of this, the same way they're left out of everything else
...
- **Two servers out of one checkout**, which is what you have if you keep a local environment up all day and run a test one beside it. Each gets its own row, its own memory limit and its own Stop button: register the second one from "other ports in use" and it's told apart from the first by its port. A project's recorded port is also what decides which of them a row stands for — before, a row matched on the directory alone and attached to whichever server was listed first, so which service its buttons acted on could change between restarts without anything on screen saying so
